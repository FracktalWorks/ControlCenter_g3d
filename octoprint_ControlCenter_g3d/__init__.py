...
class ControlCenter_g3d(octoprint.plugin.StartupPlugin):
...
"""
            ...
            try:
                ...
                if plugin_dir not in current_content or "octoprint_ControlCenter_g3d" not in current_content:
                    needs_update = True
                    self._logger.info(f"xinit file does not contain correct path to {plugin_dir}")
                else:
                    self._logger.info("xinit file already contains correct ControlCenter_g3d path")
    ...
    def get_update_information(self):
        return dict(
            ControlCenter_g3d=dict(
                displayName="ControlCenter_g3d",
                displayVersion=self._plugin_version,
                # version check: github repository
                type="github_release",
                user="FracktalWorks",
                repo="ControlCenter_g3d",
                current=self._plugin_version,

                # update method: pip
                pip="https://github.com/FracktalWorks/ControlCenter_g3d/archive/{target_version}.zip"
            )
        )


__plugin_name__ = "ControlCenter_g3d"
